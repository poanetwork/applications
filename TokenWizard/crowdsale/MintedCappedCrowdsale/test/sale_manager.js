...
contract('#MintedCappedSaleManager', function (accounts) {
      ...
      context('such as not tier 0', async () => {
        ...
        describe('events', async () => {
          ...
          describe('the other event', async () => {
            ...
            it('should match the exec id and tier index for the other topics', async () => {
              web3.toDecimal(eventTopics[1]).should.be.eq(web3.toDecimal(executionID))
              web3.toDecimal(eventTopics[2]).should.be.eq(1)
            })
        ...
        describe('storage', async () => {
          ...
          it('should have correctly update the tier min cap', async () => {
            ...
            hexStrEquals(tierInfo[0], tierNames[0]).should.be.eq(true, web3.toAscii(tierInfo[0]))
            tierInfo[1].should.be.bignumber.eq(tierCaps[0])
            tierInfo[2].should.be.bignumber.eq(tierPrices[0])
            tierInfo[3].should.be.bignumber.eq(updateTo)
            tierInfo[4].toNumber().should.be.eq(tierDurations[0])
            tierInfo[5].should.be.eq(true)
            tierInfo[6].should.be.eq(multiTierWhitelistStat[0])
          })
        })
      })
    })
      ...
      context('and the current tier is not tier 0', async () => {
        ...
        describe('storage', async () => {
          ...
          it('should have correctly update the tier min cap', async () => {
            ...
            hexStrEquals(tierInfo[0], tierNames[1]).should.be.eq(true, web3.toAscii(tierInfo[0]))
            ...
            tierInfo[5].should.be.eq(true)
            tierInfo[6].should.be.eq(multiTierWhitelistStat[1])
          })
        })
      })
    })
  ...
  describe('#createCrowdsaleTiers', async () => {
    ...
    let multiTierNames = ['Tier A', 'Tier B', 'Tier C', 'Tier D']
    let multiTierDurations = [1000, 2000, 3000, 4000]
    let multiTierPrices = [
      web3.toWei('0.1', 'ether'),
      web3.toWei('0.2', 'ether'),
    ...
    let multiTierMins = [1000, 2000, 3000, 4000]
    let multiTierModStatus = [false, false, true, true]
    let multiTierWhitelistStat = [true, true, false, true]
        ...
        context('when the sender is the admin', async () => {
          ...
          context('and wants to add multiple tiers', async () => {
            ...
            describe('returned data', async () => {
              ...
              it('should return the correct number of storage slots written to', async () => {
                updateTierReturn[2].toNumber().should.be.eq(30)
              })
            })
            ...
            describe('storage', async () => {
              ...
              it('should have an updated end time', async () => {
                ...
                timeInfo[1].toNumber().should.be.eq(
                  ...
                  + multiTierDurations[1] + multiTierDurations[2] + multiTierDurations[3]
                )
              })
              ...
              describe('Tier D (Fourth added tier)', async () => {
                ...
                it('should return valid information about tier 4', async () => {
                  ...
                  hexStrEquals(tierFourInfo[0], multiTierNames[3]).should.be.eq(true)
                  web3.fromWei(tierFourInfo[1].toNumber(), 'wei').should.be.eq(multiTierCaps[3])
                  web3.fromWei(tierFourInfo[2].toNumber(), 'wei').should.be.eq(multiTierPrices[3])
                  tierFourInfo[3].toNumber().should.be.eq(multiTierMins[3])
                  tierFourInfo[4].toNumber().should.be.eq(multiTierDurations[3])
                  tierFourInfo[5].should.be.eq(multiTierModStatus[3])
                  tierFourInfo[6].should.be.eq(multiTierWhitelistStat[3])
                })
              ...
              it('should have a tier list of length 5', async () => {
                ...
                hexStrEquals(tierListInfo[4], multiTierNames[3]).should.be.eq(true)
              })
            })
          })
          ...
          context('and wants to add multiple tiers, consecutively', async () => {
            ...
            describe('events', async () => {
              ...
              describe('second update', async () => {
                ...
                describe('the other event', async () => {
                  ...
                  it('should match the new total number of tiers for the data field', async () => {
                    web3.toDecimal(eventData).should.be.eq(6)
                  })
                })
              })
            })
            ...
            describe('storage', async () => {
              ...
              it('should have an updated end time', async () => {
                ...
                timeInfo[1].toNumber().should.be.eq(
                  startTime + initialTierDuration + multiTierDurations[0]
                  + multiTierDurations[1] + multiTierDurations[2] + multiTierDurations[3] + singleTierDuration[0]
                )
              })
              ...
              describe('Tier E (Fifth added tier)', async () => {

                it('should return valid information about tier 5', async () => {

                  let tierFiveInfo = await saleIdx.getCrowdsaleTier.call(
                    storage.address, executionID, 5
                  ).should.be.fulfilled
                  tierFiveInfo.length.should.be.eq(7)

                  hexStrEquals(tierFiveInfo[0], singleTierNames[0]).should.be.eq(true)
                  web3.fromWei(tierFiveInfo[1].toNumber(), 'wei').should.be.eq(singleTierCap[0])
                  web3.fromWei(tierFiveInfo[2].toNumber(), 'wei').should.be.eq(singleTierPrice[0])
                  tierFiveInfo[3].toNumber().should.be.eq(singleTierMin[0])
                  tierFiveInfo[4].toNumber().should.be.eq(singleTierDuration[0])
                  tierFiveInfo[5].should.be.eq(singleTierModStatus[0])
                  tierFiveInfo[6].should.be.eq(singleTierWhitelistStat[0])
                })

                it('should have correct start and end times for the tier', async () => {

                  let tierFiveTimeInfo = await saleIdx.getTierStartAndEndDates.call(
                    storage.address, executionID, 5
                  ).should.be.fulfilled
                  tierFiveTimeInfo.should.not.eq('0x')
                  tierFiveTimeInfo.length.should.be.eq(2)

                  tierFiveTimeInfo[0].toNumber().should.be.eq(
                    startTime + initialTierDuration + multiTierDurations[0]
                    + multiTierDurations[1] + multiTierDurations[2] + multiTierDurations[3]
                  )
                  tierFiveTimeInfo[1].toNumber().should.be.eq(
                    startTime + initialTierDuration + multiTierDurations[0]
                    + multiTierDurations[1] + multiTierDurations[2] + multiTierDurations[3] +
                    singleTierDuration[0]
                  )
                })
              })

              it('should have a tier list of length 6', async () => {
                let tierListInfo = await saleIdx.getCrowdsaleTierList.call(
                  storage.address, executionID
                ).should.be.fulfilled
                tierListInfo.length.should.be.eq(6)
                hexStrEquals(tierListInfo[0], initialTierName).should.be.eq(true)
                hexStrEquals(tierListInfo[1], multiTierNames[0]).should.be.eq(true)
                hexStrEquals(tierListInfo[2], multiTierNames[1]).should.be.eq(true)
                hexStrEquals(tierListInfo[3], multiTierNames[2]).should.be.eq(true)
                hexStrEquals(tierListInfo[4], multiTierNames[3]).should.be.eq(true)
                hexStrEquals(tierListInfo[5], singleTierNames[0]).should.be.eq(true)
              })
            })
          })
        })
    ...
    describe('contract storage once token is initialized', async () => {
      ...
      context('where multiple tiers were added', async () => {
        ...
        describe('storage', async () => {
          ...
          it('should have a tier list length of 5', async () => {
            let tierInfo = await saleIdx.getCrowdsaleTierList.call(
              storage.address, executionID
            ).should.be.fulfilled
            tierInfo.length.should.be.eq(5)
            ...
            hexStrEquals(tierInfo[4], multiTierNames[3]).should.be.eq(true)
          })
          ...
          it('should correctly calculate the maximum raise amount', async () => {
            ...
            let priceFive =
              web3.toBigNumber(multiTierPrices[3])
            let capFive =
              web3.toBigNumber(multiTierCaps[3])
            let raiseFive =
              priceFive.times(capFive).div(10 ** tokenDecimals)

            let totalSupply = capOne.plus(capTwo).plus(capThree).plus(capFour).plus(capFive)

            raiseInfo[0].should.be.bignumber.eq(
              raiseOne.plus(raiseTwo).plus(raiseThree).plus(raiseFour).plus(raiseFive)
            )
