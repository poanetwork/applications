...
contract('DutchCrowdsale', function (accounts) {
  ...
  it.only('should correctly set up script exec', async () => {
    ...
    envVarsContent += `${reactAppPrefix}${dutchPrefix}CROWDSALE_MANAGER${addrSuffix}='{"${networkID}":"${saleManager.address}"}'\n`
    envVarsContent += `${reactAppPrefix}${dutchPrefix}TOKEN${addrSuffix}='{"${networkID}":"${token.address}"}'\n`
    envVarsContent += `${reactAppPrefix}${dutchPrefix}TOKEN_MANAGER${addrSuffix}='{"${networkID}":"${tokenManager.address}"}'\n`
